...
#ifdef ENABLE_NVM
m_mutex_t lru_locks[POWER_LARGEST * MAX_THREAD];
/* Locks for cache LRU operations */
m_mutex_t nvm_lru_locks[POWER_LARGEST*MAX_THREAD];
#else
...
#ifdef RW_ITEM
static pthread_rwlock_t *item_locks;
#else
static m_mutex_t *item_locks;
#endif
...
#ifdef RW_ITEM
    pthread_rwlock_wrlock(&item_locks[hv & hashmask(item_lock_hashpower)]);
#else
    m_mutex_lock(&item_locks[hv & hashmask(item_lock_hashpower)]);
#endif
}

void *item_trylock(uint32_t hv) {
#ifdef RW_ITEM
    pthread_rwlock_t *lock_p = &item_locks[hv & hashmask(item_lock_hashpower)];
    if (pthread_rwlock_trywrlock(lock_p) == 0) {
        return (void *)lock_p;
    }
    return NULL;
#else
    m_mutex_t *lock_p = &item_locks[hv & hashmask(item_lock_hashpower)];
    if (m_mutex_try_lock(lock_p)) {
        return (void *)lock_p;
    }
    return NULL;
#endif
...
#else
    m_mutex_unlock((m_mutex_t *) lock);
#endif
}

void item_unlock(uint32_t hv) {
#ifdef RW_ITEM
    pthread_rwlock_unlock(&item_locks[hv & hashmask(item_lock_hashpower)]);
#else
    m_mutex_unlock(&item_locks[hv & hashmask(item_lock_hashpower)]);
#endif
}

#ifdef RW_ITEM
void item_read_lock(uint32_t hv) {
    pthread_rwlock_rdlock(&item_locks[hv & hashmask(item_lock_hashpower)]);
}
#endif
...
item *item_get(const char *key, const size_t nkey, conn *c, const bool do_update) {
    ...
    if ( it && it->refcount < 2) {
        fprintf(stderr, "(%ld), item_get: slab:%d,"
...
void item_remove_setNeedSwap(item *item) {
    ...
    item_lock(hv);
    // changes to item must be protected by lock
    item->it_flags |= ITEM_NEED_SWAP;
    do_item_remove(item);
    item_unlock(hv);
}
...
int item_replace(item *old_it, item *new_it, const uint32_t hv) {
    new_it->time = old_it->time;
    new_it->read_count = old_it->read_count;
...
void memcached_thread_init(int nthreads, void *arg) {
    ...
    for (i = 0; i < POWER_LARGEST; i++) {
        ...
        for (int j = 0; j < MAX_THREAD; ++j) {
            m_mutex_init(&lru_locks[i + j * POWER_LARGEST]);
            m_mutex_init(&nvm_lru_locks[i + j * POWER_LARGEST]);
...
#ifdef RW_ITEM
    item_locks = calloc(item_lock_count, sizeof(pthread_rwlock_t));
    if (! item_locks) {
        perror("Can't allocate item locks");
        exit(1);
    }
    for (i = 0; i < item_lock_count; i++) {
        pthread_rwlock_init(&item_locks[i], NULL);
    }
#else
item_locks = calloc(item_lock_count, sizeof(m_mutex_t));
    if (! item_locks) {
        perror("Can't allocate item locks");
        exit(1);
    }
#ifndef M_LOCK
    for (i = 0; i < item_lock_count; i++) {
        m_mutex_init(&item_locks[i]);
    }
#else
    memset((void *)item_locks, 0 , item_lock_count*sizeof(m_mutex_t));
#endif
#endif
#ifndef YCSB
