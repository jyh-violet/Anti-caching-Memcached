...
//    fprintf(stderr, "pageId:%d, off:%d, orig_ntotal:%ld\n", pageId, offset,  orig_ntotal);
    ...
    && (hdr->offset == (offset))
    && (IsWarm(old_it) || (old_it->it_flags & ITEM_NEED_SWAP))){
        item_lock(hv);
        ...
        && (hdr->offset == (offset)) ){
            ...
            }else if(IsWarm(old_it) || (old_it->it_flags & ITEM_NEED_SWAP)){
#if (!defined(NVM_AS_DRAM)) && (!defined(DRAM_CACHE))
                toNVM:
#endif
...
bool _storage_Swap_wbuf(char* buf, size_t size, int pageVersion, int pageId, int offset, int local_nvm_slab_index, int local_nvm_per_thread_counter){
    ...
    for (size_t i = 0; i <size;) {
        ...
        if(i + orig_ntotal <= size){
            uint32_t crc2 = crc32c(0,(char *)it+STORE_OFFSET, orig_ntotal-STORE_OFFSET);
            if(it->exptime != crc2){
                break;
            }
        } else{
    ...
    bool _storage_reevict_wbuf(void* storage, char* buf, size_t size, int pageVersion, int pageId, int offset){
        if(storage_write_threadId == 0){
            ...
            && (IsWarm(old_it) || (old_it->it_flags & ITEM_NEED_SWAP))){
                ...
                && (hdr->offset == (item_offset)) ){
                    ...
                    if (extstore_write_request(storage, PAGE_BUCKET_BASE, PAGE_BUCKET_DEFAULT, &io) == 0) {
                        ...
                        memcpy((char*)buf_it, (char*)it, orig_ntotal);
        ...
        while (swapBuf) {
            ...
            switch (swapBuf->mode) {
                case SWAP_ITME:
                    ...
                    crc2 = crc32c(0, (char *)it+STORE_OFFSET, orig_ntotal-STORE_OFFSET);
                    if(it->exptime == crc2){
                        adder ++;
                ...
                case SWAP_WBUF_NEEDREAD:
                    ...
                    while (readWbuf(me->engine, swapBuf->page_id, swapBuf, swapBuf->len, swapBuf->offset) < 0){
                    }
                    if(_storage_Swap_wbuf(swapBuf->buf, swapBuf->len,
                                          swapBuf->page_version, swapBuf->page_id, swapBuf->offset, local_nvm_slab_index, local_nvm_per_thread_counter) == false){
                        resetPageWbufWaitSwap(me->engine, swapBuf->page_id,swapBuf->offset);
    ...
    while (1) {
...
#if defined(NO_SSD) || defined(DRAM_CACHE)
        ...
        if (nvm_limit_reached){
            ...
            for (int x = 1; x <MAX_NUMBER_OF_SLAB_CLASSES; x++) {
                ...
                if( nvm_limit_reached && (((target > (last_target[x])) || (chunks_free < target / 2) || (chunks_free < (last_chunks_free[x] /2))))){
                    // storage_write() will fail and cut loop after filling write buffer.
                    int failed = 0;

                    while ((chunks_free + toSSD < target)) {
                        int ret = 0;
...
#endif
    cf->ext_cf.bufSwapFn = _storage_reevict_wbuf;
...
bool flushItToExt(void * storage, struct lru_pull_tail_return *it_info, item *hdr_it, obj_io* pre_io){
    ...
    if (extstore_write_request(storage, PAGE_BUCKET_BASE, PAGE_BUCKET_DEFAULT, &io) == 0) {
        ...
        // copy from past the headers + time headers.
        // TODO: should be in items.c
        memcpy((char *)io.buf+STORE_OFFSET, (char *)((item_hdr *) ITEM_data(it))->nvmptr+STORE_OFFSET, io.len-STORE_OFFSET);
        buf_it->nbytes = it->nbytes;
        buf_it->nkey = it->nkey;
        buf_it->slabs_clsid = it->slabs_clsid;
        // crc what we copied so we can do it sequentially.
...
it_info.it = NULL;
        ...
        item_hdr * hdr =  (item_hdr *)ITEM_data(hdr_it);
        if(((hdr_it->it_flags & ITEM_NVM) || (hdr_it->it_flags & ITEM_HDR)) && hdr->cache_item == it){
            hdr->cache_item = NULL;
            do_item_remove(it);
        } else{
            fprintf(stderr, "dram_evict error, hdr_it:%p, cache_item:%p, it:%p\n",
                    (void *)hdr_it, (void *)hdr->cache_item, (void *)it);
            exit(0);
        }
        do_item_remove(hdr_it);
        do_item_remove(it);
    ...
    for (int i = 0; i < local_slabNum && hdr_it == NULL; ++i) {
        ...
        hdr_it = slabs_alloc_c(orig_ntotal, clsid | NVM_SLAB, 0, slab_counter, local_slabNum);
    }
    if(false){
...
//    int need_sleep[MAX_NUMBER_OF_SLAB_CLASSES] = {0};
    ...
    while (1) {
        ...
        for (int x = POWER_SMALLEST; x < MAX_NUMBER_OF_SLAB_CLASSES; x++) {
            ...
            unsigned int toSSD = 0;
            int failed = 0;
            if(mem_limit_reached && ((target > last_target[x]) || (chunks_free < target / 3) || (chunks_free < last_chunks_free[x] / 2))){
                // storage_write() will fail and cut loop after filling write buffer.
                while (((chunks_free + toNVM < target))) {
                    int ret = 0;
                    local_lru_index = (local_lru_index + 1) % local_per_thread_counter;
