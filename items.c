...
#ifdef ENABLE_NVM
extern m_mutex_t lru_locks[POWER_LARGEST * MAX_THREAD];

extern m_mutex_t nvm_lru_locks[POWER_LARGEST*MAX_THREAD];

extern bool nvm_limit_reached;
#else
...
#ifdef ENABLE_NVM
static item *heads[LARGEST_ID * MAX_THREAD];
static item *tails[LARGEST_ID * MAX_THREAD];
static itemstats_t itemstats[LARGEST_ID * MAX_THREAD];
static unsigned int sizes[LARGEST_ID * MAX_THREAD];
static uint64_t sizes_bytes[LARGEST_ID * MAX_THREAD];
static item *nvm_heads[LARGEST_ID* MAX_THREAD];
static item *nvm_tails[LARGEST_ID* MAX_THREAD];
#ifdef DEBUG
static uint64_t nvm_sizes_bytes[LARGEST_ID* MAX_THREAD];
#endif
static unsigned int nvm_sizes[LARGEST_ID* MAX_THREAD];
extern int alloc_pull_flags[MAX_THREAD];
extern  int nvm_alloc_pull_flags[MAX_THREAD];

__thread int lru_counter = 0;
__thread int lru_index = 0;
__thread int nvm_lru_counter = 0;
#else
...
void item_stats_reset(void) {
    ...
    for (i = 0; i < LARGEST_ID; i++) {
        m_mutex_lock(&lru_locks[i]);
        memset(&itemstats[i], 0, sizeof(itemstats_t));
        m_mutex_unlock(&lru_locks[i]);
    }
}
...
__thread int i_counter = 0;
extern int slabNum;
extern __thread int slab_index;
extern __thread enum THREAD_TYPE thread_type;
extern __thread int slab_counter_index;
#define COUNTER_TRACE 100
extern bool thread_finish[MAX_THREAD];
...
#ifdef NO_FLUSH_IN_ALLOC
    int slab_counter = 0;
    int local_slabNum = slabNum;

#ifdef SLAB_PART
    int local_per_thread_counter = per_thread_counter;
    slab_counter_index = (slab_counter_index + 1) % local_per_thread_counter;
    int local_slab_counter_index = slab_counter_index;
    for (i = 0; i < local_per_thread_counter * 2; i++) {
        slab_counter = (slab_index + (local_slab_counter_index + i)%local_per_thread_counter);
#else
    for (i = 0; i < 10; i++) {
#endif
#else
...
#ifdef ENABLE_NVM
            struct lru_pull_tail_return it_info = {0, 0};
            lru_pull_tail(id, COLD_LRU, 0, LRU_PULL_RETURN_ITEM | LRU_PULL_MEM_ONLY, 0, &it_info);
            if (settings.lru_segmented) {
                lru_pull_tail(id, HOT_LRU, 0, 0, 0, NULL);
                lru_pull_tail(id, WARM_LRU, 0, 0, 0, NULL);
            }
            if(it_info.it != NULL){
                if(flushItToNvm(&it_info)){
                }
            }
...
#ifdef SLAB_PART
    if(it == NULL && (i_counter % COUNTER_TRACE == COUNTER_TRACE - 1)){

        for (i = 0; i < local_slabNum; i++) {
            int slab_counter =((slab_index + (local_slab_counter_index + i)) % slab_span + slab_min);
            it = slabs_alloc_c(ntotal, id, 0, slab_counter, local_slabNum);
            if( it != NULL){
                break;
            }
...
item *do_item_alloc(char *key, const size_t nkey, const unsigned int flags,
    ...
    it->next = it->prev = 0;
//    fprintf(stderr, "do_item_alloc: it->next = NULL, it:%p, id:%d\n", (void *)it, id);
...
#ifdef ENABLE_NVM
#ifdef LRU_COUNTER
    lru_index  = (lru_index + 1) % per_thread_counter;
    lru_counter = (slab_index + lru_index) % slabNum;
#ifdef use_alloc_pull_flags
    if(alloc_pull_flags[lru_counter] == 1){
        lru_counter += slabNum;
    }
#endif
#endif
    id |= (lru_counter * POWER_LARGEST);
#endif
    it->slabs_clsid = id;
...
item *do_item_alloc_nvm(char *key, const size_t nkey, const unsigned int flags, const int nbytes) {
    uint16_t nsuffix;
    item *it = NULL;
    char suffix[40];
    // Avoid potential underflows.
     ...
     *  chunks while reading the upload.
     */
    it = do_item_alloc_pull(ntotal, id | NVM_SLAB);
    if (it == NULL) {
        i_counter ++;
#ifndef ENABLE_NVM
        pthread_mutex_lock(&lru_locks[id]);
...
#ifdef ENABLE_NVM
    assert(it != nvm_heads[it->slabs_clsid]);
    assert(it != nvm_tails[it->slabs_clsid]);
#endif
...
#ifdef FIX_KEY
        slabs_free(it, ntotal, clsid | NVM_SLAB);
#else
...
static void do_item_link_q(item *it) { /* item is the new head */
    ...
    if(it->it_flags & ITEM_NVM){
#ifdef FIX_KEY
        head = &nvm_heads[it->slabs_clsid];
        tail = &nvm_tails[it->slabs_clsid];
        assert(it != *head);
        ...
        *head = it;
        if (*tail == 0) *tail = it;
//        fprintf(stderr, "do_item_link_q: %p, flag:%d,  slab:%d, tail:%p, prev:%p, next:%p,nvm_sizes:%d\n",
//                (void*)it, it->it_flags, it->slabs_clsid,  (void*) nvm_tails[it->slabs_clsid], (void*)(it->prev), (void*)(it->next), nvm_sizes[it->slabs_clsid]);
#ifdef DEBUG
        nvm_sizes[it->slabs_clsid]++;
        nvm_sizes_bytes[it->slabs_clsid] += (ITEM_ntotal(it) - it->nbytes) + sizeof(item_hdr);
#endif
//        fprintf(stderr, "do_item_link_q(ITEM_NVM): slabs_clsid:%d, nvm_sizes:%d\n", it->slabs_clsid, nvm_sizes[it->slabs_clsid]);

#else
        item_hdr* hdr = (item_hdr*) ITEM_data(it);
//        fprintf(stderr, "do_item_link_q: nvm_slab_id:%d\n", (hdr)->nvm_slab_id);
        head = &nvm_heads[(hdr)->nvm_slab_id];
        tail = &nvm_tails[(hdr)->nvm_slab_id];
        assert(it != *head);
        assert((*head && *tail) || (*head == 0 && *tail == 0));
        it->prev = 0;
        it->next = *head;
//        if(it->next == NULL){
//        fprintf(stderr, "do_item_link_q  nvm_slab_id:%d, size:%d\n", hdr->nvm_slab_id, nvm_sizes[hdr->nvm_slab_id]);
//        }
        if (it->next) it->next->prev = it;
        *head = it;
        if (*tail == 0) *tail = it;
//        fprintf(stderr, "do_item_link_q: %p, flag:%d,  tail:%p, prev:%p, next:%p,\n",
//                (void*)it, it->it_flags,  (void*) nvm_tails[(hdr)->nvm_slab_id], (void*)(it->prev), (void*)(it->next));
        nvm_sizes[hdr->nvm_slab_id]++;
        nvm_sizes_bytes[hdr->nvm_slab_id] += (ITEM_ntotal(it) - it->nbytes) + sizeof(item_hdr);
        it->it_flags |= ITEM_LINKED;
#endif
        return;
    }
    ...
    head = &heads[slab_clsid];
    tail = &tails[slab_clsid];
    assert(it != *head);
    assert((*head && *tail) || (*head == 0 && *tail == 0));
    it->prev = 0;
    it->next = *head;
...
#ifdef DEBUG
    sizes[slab_clsid]++;
//    fprintf(stderr, "(%ld), do_item_link_q slab_clsid:%d, it:%p, next:%p, tail:%p\n",
//            pthread_self(), it->slabs_clsid, (void *)it, (void *)it->next, (void *)*tail);
#ifdef EXTSTORE
    if ((it->it_flags & ITEM_HDR) || (it->it_flags & ITEM_NVM)) {
        sizes_bytes[slab_clsid] += (ITEM_ntotal(it) - it->nbytes) + sizeof(item_hdr);
    } else {
        sizes_bytes[slab_clsid] += ITEM_ntotal(it);
    }
#else
    sizes_bytes[slab_clsid] += ITEM_ntotal(it);
#endif
...
//    fprintf(stderr, "item_link_q slab:%d, flag:%d\n", it->slabs_clsid, it->it_flags);

#ifdef ENABLE_NVM
#ifdef FIX_KEY
    m_mutex_t *lock_p = (it->it_flags & ITEM_NVM)?
        &nvm_lru_locks[it->slabs_clsid]:
        &lru_locks[it->slabs_clsid];
#else
    m_mutex_t *lock_p = (it->it_flags & ITEM_NVM)?
            &nvm_lru_locks[((item_hdr*) ITEM_data(it))->nvm_slab_id]:
            &lru_locks[it->slabs_clsid];
#endif
//    fprintf(stderr, "before item_link_q apply slab:%d, flag:%d\n", it->slabs_clsid, it->it_flags);
    m_mutex_lock(lock_p);
//    fprintf(stderr, "item_link_q apply slab:%d, flag:%d\n", it->slabs_clsid, it->it_flags);

#else
pthread_mutex_lock(&lru_locks[it->slabs_clsid]);
#endif
    do_item_link_q(it);
    if(it->it_flags & ITEM_SLABBED || it->refcount < 1){
        fprintf(stderr, "after item_link_q(%ld): %p, flag:%d, ref:%d class_id:%d is slabbed\n",
                pthread_self(), (void *)it, it->it_flags, it->refcount, it->slabs_clsid);
        BACKTRACE_T;
        exit(0);
    }
#ifdef ENABLE_NVM
...
pthread_mutex_lock(&lru_locks[it->slabs_clsid]);
    ...
    do_item_link_q(it);
    if(it->it_flags & ITEM_SLABBED || it->refcount < 1){
        fprintf(stderr, "after item_link_q_incr(%ld): %p, flag:%d, ref:%d class_id:%d is slabbed\n",
                pthread_self(), (void *)it, it->it_flags, it->refcount, it->slabs_clsid);
        BACKTRACE_T
        exit(0);
    }
    refcount_incr(it);
    if(it->refcount < 2){
        fprintf(stderr, "item_link_q_incr new_it:%p, flag:%d, ref:%d\n",
...
static void item_link_q_warm(item *it) {
#ifdef ENABLE_NVM
    fprintf(stderr, "ENABLE_NVM set, should not call item_link_q_warm error\n");
    exit(0);
    if(it->it_flags& ITEM_NVM){
        return;
    }
    ...
    int slab_clsid = it->slabs_clsid;
#endif
    m_mutex_lock(&lru_locks[slab_clsid]);
    do_item_link_q(it);
    itemstats[slab_clsid].moves_to_warm++;
    m_mutex_unlock(&lru_locks[slab_clsid]);
}
...
static void do_item_unlink_q(item *it) {
    ...
    if(it->it_flags & ITEM_NVM){
        ...
        //        fprintf(stderr, "do_item_unlink_q: %p, tail:%p, prev:%p, next:%p,\n",
        //                (void*)it, (void*) nvm_tails[(hdr)->nvm_slab_id], (void*)(it->prev), (void*)(it->next));
        head = &nvm_heads[it->slabs_clsid];
        tail = &nvm_tails[it->slabs_clsid];
        if (*head == it) {
            assert(it->prev == 0);
...
//                (void*)it, (void*) nvm_tails[(hdr)->nvm_slab_id], (void*)(it->prev), (void*)(it->next));
        ...
        if (it->prev) it->prev->next = it->next;
//        if(it->next == NULL){
//        fprintf(stderr, "%d, do_item_unlink_q: nvm_slab_id:%d, nvm_sizes:%d, \n",
//            __LINE__, (hdr)->nvm_slab_id, nvm_sizes[hdr->nvm_slab_id]);
////        }
        nvm_sizes[hdr->nvm_slab_id]--;
        nvm_sizes_bytes[hdr->nvm_slab_id] -= (ITEM_ntotal(it) - it->nbytes) + sizeof(item_hdr);
        return;
    ...
    if (it->next) it->next->prev = it->prev;
    if (it->prev) it->prev->next = it->next;
//    fprintf(stderr, "(%ld), do_item_unlink_q, slab_clsid:%d, it:%p, next:%p, tail:%p\n",
//            pthread_self(), it->slabs_clsid, (void *)it, (void *)it->next, (void *)*tail);
//    if(it->prev){
//        fprintf(stderr, "(%ld), do_item_unlink_q, slab_clsid:%d, it:%p, next:%p\n",
//                pthread_self(), it->prev->slabs_clsid, (void *)it->prev, (void *)it->prev->next);
//    }
#ifdef DEBUG
    sizes[slab_clsid]--;
#ifdef EXTSTORE
...
void item_unlink_q(item *it) {
    ...
    if(it->it_flags & ITEM_HDR){
        fprintf(stderr, "item_unlink_q:%p, flag:%d, HDR\n", (void *)it, it->it_flags);
        BACKTRACE_T
        return;
    }
#endif
...
int item_try_unlink_q(item *it) {
    if(it->it_flags & ITEM_NVM) {
            fprintf(stderr, "before item_try_unlink_q(%ld): nvm item:%d\n", pthread_self(), it->it_flags);
            BACKTRACE_T
            exit(0);
    }
...
int do_item_link(item *it, const uint32_t hv) {
    ...
    if(it->it_flags & ITEM_SLABBED){
        fprintf(stderr, "before do_item_link(%ld): %p, flag:%d, ref:%d is slabbed\n", pthread_self(), (void *)it, it->it_flags, it->refcount);
        BACKTRACE_T
        exit(0);
    }
#ifndef ENABLE_NVM
    ...
    assoc_insert(it, hv);
    item_link_q_incr(it);
    if(it->it_flags & ITEM_SLABBED || it->refcount < 2){
        fprintf(stderr, "after do_item_link(%ld): %p, flag:%d, ref:%d class_id:%d ,  nvmptr:%p,  slab_index:%d is slabbed\n",
                pthread_self(), (void *)it, it->it_flags, it->refcount, it->slabs_clsid,
                (void *)((item_hdr*) ITEM_data(it))->nvmptr, slab_index);
        BACKTRACE_T
        exit(0);
    }
#ifndef ENABLE_NVM
...
void do_item_remove_with_unlock(item *it, uint32_t hv) {
    MEMCACHED_ITEM_REMOVE(ITEM_key(it), it->nkey, it->nbytes);
    assert((it->it_flags & ITEM_SLABBED) == 0);
    assert(it->refcount > 0);
    ...
    if (settings.lru_segmented) {
        ...
        if ((it->it_flags & ITEM_LINKED) != 0) {
            if (ITEM_lruid(it) == COLD_LRU && (it->it_flags & ITEM_ACTIVE)) {
                it->time = current_time;
                item_unlink_q(it);
                it->slabs_clsid = ITEM_clsid(it) | WARM_LRU;
...
int do_item_replace(item *it, item *new_it, const uint32_t hv) {
    ...
    if(it->it_flags & ITEM_LINKED){
        fprintf(stderr, "do_item_replace, it:%p, flag:%d, ref:%d\n", (void *)it, it->it_flags, it->refcount);
    }
...
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv, conn *c, const bool do_update) {
    ...
    if (it != NULL) {
        if ( false) {
            fprintf(stderr, "(%ld), do_item_get: slab:%d,"
                            "it:%p,  it->flag:%d,  ref:%d\n",
                            pthread_self(),  it->slabs_clsid,
                            (void *)it,  it->it_flags, it->refcount);
        }
#ifdef RW_ITEM
        // the item is protected by read lock, and the increase must in atomic
        __sync_fetch_and_add (&it->refcount, 1);
#else
        refcount_incr(it);
        if(it->refcount < 2){
            fprintf(stderr, "do_item_get new_it:%p, flag:%d, ref:%d\n",
                    (void *)it, it->it_flags, it->refcount);
        }
#endif
...
int lru_pull_tail(const int orig_id, const int cur_lru,
    ...
    if(flags & LRU_NVM){
        pthread_mutex_lock(&nvm_lru_locks[id]);
        search = nvm_tails[id];
        //        if(search != NULL){
        //            fprintf(stderr, "lru_pull_tail, slab:%d,  nvm_sizes:%d\n", id, nvm_sizes[id]);
        //        }
    ...
    for (; tries > 0 && search != NULL; tries--, search=next_it, hold_lock = NULL) {
        ...
        if((flags & LRU_PULL_RETURN_ITEM) && ((search->it_flags & ITEM_HDR )
        || ((flags & LRU_PULL_MEM_ONLY) && (search->it_flags & ITEM_NVM)))){
            if(search->it_flags & ITEM_HDR){
                fprintf(stderr, "lru_pull_tail: %p, flag:%d, tail:%p\n",
                        (void*)search, search->it_flags, (void *)nvm_tails[id]);
                exit(0);
            }
            continue;
        }
        if(((flags & LRU_PULL_NVM_ONLY) && !(search->it_flags & ITEM_NVM))){
            break;
        }



        hv = hash(ITEM_key(search), search->nkey);
        ...
        if( (flags & LRU_NVM ) && (!(search->it_flags & ITEM_NVM))){
    ...
    if(hold_lock == NULL){
        if(flags & LRU_NVM){
            pthread_mutex_unlock(&nvm_lru_locks[id]);
        } else{
            pthread_mutex_unlock(&lru_locks[id]);
        }
        return removed;
...
int lru_pull_tail(const int orig_id, const int cur_lru,
    ...
    if(flags & LRU_NVM){
        id |= (nvm_lru_counter * POWER_LARGEST);
        m_mutex_lock(&nvm_lru_locks[id]);
        search = nvm_tails[id];

    } else{
        id |= cur_lru | (lru_counter * POWER_LARGEST);
        m_mutex_lock(&lru_locks[id]);
        search = tails[id];
    }
    /* We walk up *only* for locked items, and if bottom is expired. */
...
//                pthread_self(), cur_lru, flags, (void *)search, id, search->slabs_clsid);
        ...
        if((flags & LRU_PULL_CRAWL_BLOCKS) && (flags & LRU_NVM) && !IsHot(search)){
            continue;
        }
        ...
        if(search->refcount < 2){
            fprintf(stderr, "lru_pull_tail new_it:%p, flag:%d, ref:%d\n",
                    (void *)search, search->it_flags, search->refcount);
        }
#endif
        if(search->read_count >= 1){
//            search->read_count --;
        }
        /* If we're HOT_LRU or WARM_LRU and over size limit, send to COLD_LRU.
         * If we're COLD_LRU, send to WARM_LRU unless we need to evict
        ...
        switch (cur_lru) {
...
#else
                    limit = total_bytes * ((cur_lru == HOT_LRU) ?settings.hot_lru_pct : settings.warm_lru_pct )/ 100;
                    /* Rescue ACTIVE items aggressively */
                    if ((search->it_flags & ITEM_ACTIVE) != 0) {
                        search->it_flags &= ~ITEM_ACTIVE;
                        removed++;
            ...
            case COLD_LRU:
                ...
                if (flags & LRU_PULL_RETURN_ITEM) {
                    ...
                    /*
                    ret_it->nkey = it->nkey;
                    ret_it->slabs_clsid = ITEM_clsid(it);
                    ret_it->it_flags = it->it_flags;
                    memcpy(ret_it->key, ITEM_key(it), it->nkey);*/
//                    fprintf(stderr, "(%ld), lru_pull_tail: it:%p, lru_counter:%d, id:%d\n", pthread_self(), (void *)ret_it->it, lru_counter, id);
                } else if (IsHot(search)
                        && settings.lru_segmented) {
#ifdef ENABLE_NVM
                    move_to_lru = HOT_LRU;
                    do_item_unlink_q(search);
    ...
    if (it != NULL) {
        if (move_to_lru >= 0) {
            it->slabs_clsid = ITEM_clsid(it) | move_to_lru | (lru_counter * POWER_LARGEST);
            item_link_q(it);
        }
        // make sure the item will not be removed and recycled
        if(!(flags & LRU_PULL_RETURN_ITEM)){
            do_item_remove(it);
            item_trylock_unlock(hold_lock);
        }
    ...
    int lru_pull_tail_hot(const int orig_id, const int cur_lru,
                      const uint64_t total_bytes, const uint8_t flags, const rel_time_t max_age,
                      struct lru_pull_tail_return *ret_it) {
        item *it = NULL;
        int id = orig_id;
        ...
        item *next_it;
        void *hold_lock = NULL;
        assert(((flags & LRU_NVM) == 0) && ((flags & LRU_PULL_RETURN_ITEM) != 0) && (cur_lru == HOT_LRU));
        id |= cur_lru | (lru_counter * POWER_LARGEST);
        m_mutex_lock(&lru_locks[id]);
        search = tails[id];
        ...
        for (; tries > 0 && search != NULL; tries--, search=next_it) {
            ...
            assert(!((search->it_flags & (ITEM_NVM | ITEM_HDR))));
            if(false ){
            ...
            refcount_incr(search);
            if(search->refcount < 2){
                fprintf(stderr, "lru_pull_tail_hot new_it:%p, flag:%d, ref:%d\n",
                        (void *)search, search->it_flags, search->refcount);
            }
            /* If we're HOT_LRU or WARM_LRU and over size limit, send to COLD_LRU.
             * If we're COLD_LRU, send to WARM_LRU unless we need to evict
...
int lru_pull_head(const int orig_id, const int cur_lru,
                  const uint64_t total_bytes, const uint8_t flags, const rel_time_t max_age,
                  struct lru_pull_tail_return *ret_it) {
    item *it = NULL;
    int id = orig_id;
    ...
    if((!(flags & LRU_PULL_RETURN_ITEM)) || !(flags & LRU_NVM)){
        fprintf(stderr, "(%ld), lru_pull_head, flag:%d error \n", pthread_self(), flags);
        return 0;
    }
    id |= (nvm_lru_counter * POWER_LARGEST);
    m_mutex_lock(&nvm_lru_locks[id]);
    ...
    for (; tries > 0 && search != NULL; tries--, search=next_it) {
        ...
        next_it = search->next;
#ifdef FIX_KEY
        if(search->slabs_clsid != id){
            fprintf(stderr, "(%ld), lru_pull_head(cur_lru:%d, flag:%d)  "
                            "search:%p, id:%d, serach->id:%d, search->flag:%d, key:%.30s error\n",
                            pthread_self(), cur_lru, flags, (void *)search, id,
                            search->slabs_clsid, search->it_flags, ITEM_key(search));
#else
        if(((item_hdr*) ITEM_data(search))->nvm_slab_id != id){
            fprintf(stderr, "(%ld), lru_pull_head(cur_lru:%d, flag:%d)  "
                            "search:%p, id:%d, serach->id:%d, search->flag:%d, key:%.30s error\n",
                            pthread_self(), cur_lru, flags, (void *)search, id,
                            ((item_hdr*) ITEM_data(search))->nvm_slab_id, search->it_flags, ITEM_key(search));
#endif
            exit(-1);
        }

        if(((flags & (LRU_PULL_NVM_ONLY|LRU_NVM)) && !(search->it_flags & ITEM_NVM))){
            break;
        }
        if(!IsHotForSwap(search)){
            continue;
        }
        ...
        refcount_incr(search);
        if(search->refcount < 2){
            fprintf(stderr, "lru_pull_head new_it:%p, flag:%d, ref:%d\n",
                    (void *)search, search->it_flags, search->refcount);
        }
        it = search;
        ret_it->it = it;
    ...
    m_mutex_unlock(&nvm_lru_locks[id]);
    return removed;
}
...
static int lru_maintainer_juggle(const int slabs_clsid) {
    ...
    slabs_available_chunks(slabs_clsid, NULL,
            &chunks_perslab, slabNum, 0, slabNum);
#ifdef ENABLE_NVM
    if(settings.ext_free_memchunks[slabs_clsid] == 0){
        return did_moves;
    }
#endif
    if (settings.temp_lru) {
    ...
    if (settings.lru_segmented) {
        m_mutex_lock(&lru_locks[slabs_clsid|COLD_LRU]);
        ...
        m_mutex_unlock(&lru_locks[slabs_clsid|COLD_LRU]);
        ...
        m_mutex_lock(&lru_locks[slabs_clsid|HOT_LRU]);
        total_bytes += sizes_bytes[slabs_clsid|HOT_LRU];
        m_mutex_unlock(&lru_locks[slabs_clsid|HOT_LRU]);

        m_mutex_lock(&lru_locks[slabs_clsid|WARM_LRU]);
        total_bytes += sizes_bytes[slabs_clsid|WARM_LRU];
        m_mutex_unlock(&lru_locks[slabs_clsid|WARM_LRU]);
    }
#else
    rel_time_t hot_age = 5;
    total_bytes = 10;
#endif
#ifdef LRU_COUNTER
    int local_slabNum = slabNum;
...
static int lru_nvm_maintainer_juggle(const int slabs_clsid) {
    int i;
    int did_moves = 0;
    uint64_t total_bytes = 0;
    unsigned int chunks_perslab = 0;
    ...
    //unsigned int chunks_free = 0;
    /* TODO: if free_chunks below high watermark, increase aggressiveness */
    slabs_available_chunks(slabs_clsid + MAX_NUMBER_OF_SLAB_CLASSES, NULL,
                           &chunks_perslab, local_slabNum, 0, local_slabNum);
    if(settings.ext_free_memchunks[slabs_clsid + MAX_NUMBER_OF_SLAB_CLASSES] == 0){
        return did_moves;
    }
    ...
    /* Juggle HOT/WARM up to N times */
    for (i = 0; i < 500; i++) {
        nvm_lru_counter  = i % local_slabNum;
        if(lru_pull_tail(slabs_clsid, HOT_LRU, total_bytes, LRU_PULL_CRAWL_BLOCKS | LRU_NVM, hot_age, NULL) == 0){
            break;
        }
        did_moves++;
    }
...
static void *lru_maintainer_thread(void *arg) {
    thread_type = LRU_MAINTAIN_THREAD;
    ...
    while (do_run_lru_maintainer_thread) {
        did_moves = 0;
//        usleep(MIN_LRU_MAINTAINER_SLEEP);
        for (i = 1; i <= MAX_NUMBER_OF_SLAB_CLASSES; i++) {
            if (get_slab_size(i) < MIN_VALUE_SIZE){
                continue;
            }
        ...
        if (settings.slab_automove == 1 && last_automove_check != current_time) {
            if (last_ratio != settings.slab_automove_ratio) {
                ...
                nvm_slab_automove_extstore_free(nvm_am);
                nvm_am = nvm_slab_automove_extstore_init(&settings);
            ...
            nvm_slab_automove_extstore_run(am, &src, &dst);
            if (src != -1 && dst != -1) {
                slabs_reassign(src, dst);
                LOGGER_LOG(l, LOG_SYSEVENTS, LOGGER_SLAB_MOVE, NULL,
                           src, dst);
    ...
    static void *lru_nvm_maintainer_thread(void *arg) {
        thread_type = LRU_MAINTAIN_THREAD;
        slab_index = 0;
        per_thread_counter = slabNum;
        int i;
        ...
        while (do_run_lru_maintainer_thread) {
            ...
            //        usleep(MIN_LRU_MAINTAINER_SLEEP);
            for (i = 1; i <= MAX_NUMBER_OF_SLAB_CLASSES; i++) {
                // nvm slab size is same to the one in dram
                if (get_slab_size(i) < MIN_VALUE_SIZE){
                    continue;
                }
                did_moves +=lru_nvm_maintainer_juggle(i);
            }
            if(did_moves == 0){
                usleep(10000); // 10ms
            }
            ...
            // LRU crawler *must* be stopped.
            if (settings.verbose > 2)
                fprintf(stderr, "LRU nvm maintainer thread stopping\n");
...
int stop_lru_maintainer_thread(void) {
    ...
    if ((ret = pthread_join(lru_nvm_maintainer_tid, NULL)) != 0) {
        fprintf(stderr, "Failed to stop LRU nvm maintainer thread: %s\n", strerror(ret));
        return -1;
    }
...
int start_lru_maintainer_thread(void *arg) {
    ...
    if ((ret = pthread_create(&lru_nvm_maintainer_tid, NULL,
                              lru_nvm_maintainer_thread, arg)) != 0) {
...
void do_item_linktail_q(item *it) { /* item is the new tail */
    ...
    it->prev = *tail;
    it->next = 0;
//    fprintf(stderr, "do_item_linktail_q: it->next = NULL, it:%p\n", (void *)it);
    if (it->prev) {
...
item *do_item_crawl_q(item *it) {
    ...
    if (it->prev) {
        ...
        if (it->next) {
            ...
            it->prev->next = 0;
//            fprintf(stderr, "do_item_crawl_q: it->next = NULL, it:%p\n", (void *)it->prev);
        }
...
//    }
 ...
 void print_slab_tail(int  slab_id){
    fprintf(stderr, "print_slab_tail: slab:%d, index:%d size[hot]:%d,size[warm]:%d,size[cold]:%d\n",
            slab_id, slab_id / POWER_LARGEST, sizes[slab_id | HOT_LRU], sizes[slab_id | WARM_LRU],sizes[slab_id | COLD_LRU]);
}

void print_nvm_slab_tail(int  slab_id){
    fprintf(stderr, "print_nvm_slab_tail: slab:%d, index:%d, size[hot]:%d\n",
            slab_id, slab_id / POWER_LARGEST,nvm_sizes[slab_id | HOT_LRU]);
}

int get_size(int slab_id){
#ifdef SLAB_PART
    int size = 0;
    for (int i = 0; i < slabNum * 2; ++i) {
        size += sizes[slab_id | (i * POWER_LARGEST)];
    }
    return size;
#else
