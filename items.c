...
void do_item_bump(conn *c, item *it, const uint32_t hv) {
    ...
    if (settings.lru_segmented) {
        if(it->time == current_time){
            if(it->read_count < (HOT_ITEM_R_COUNT << 2)){
                it->read_count ++;
            }
        } else{
...
int lru_pull_tail(const int orig_id, const int cur_lru,
    ...
    for (; tries > 0 && search != NULL; tries--, search=next_it) {
        ...
        if(false){
            fprintf(stderr, "(%ld), lru_pull_tail(cur_lru:%d, flag:%d) "
                            "search:%p, id:%d, serach->id:%d, search->flag:%d, hdr->id:%d key:%.30s, error\n",
                            pthread_self(), cur_lru, flags, (void *)search, id, search->slabs_clsid, search->it_flags,
                            ((item_hdr*)(ITEM_data(search)))->nvm_slab_id, ITEM_key(search));
            exit(0);
            continue;
        }
