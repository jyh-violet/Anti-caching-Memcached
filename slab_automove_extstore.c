...
#ifdef ENABLE_NVM
        if (a->settings->ext_free_memchunks[n] != hold_free && sam->total_pages > 0) {
            a->settings->ext_free_memchunks[n] = hold_free;
//            fprintf(stderr, "memcheck, ext_free_memchunks[%d]:%d, total_pages:%ld, chunks_per_page:%d, chunk_size:%d\n",
//                    n, hold_free, sam->total_pages, sam->chunks_per_page, sam->chunk_size);
        }
...
#ifdef ENABLE_NVM
void *nvm_slab_automove_extstore_init(struct settings *settings) {
    uint32_t window_size = settings->slab_automove_window;
    double max_age_ratio = settings->slab_automove_ratio;
    slab_automove *a = calloc(1, sizeof(slab_automove));
    if (a == NULL)
    ...
    // do a dry run to fill the before structs
    nvm_fill_slab_stats_automove(a->sam_before);

    return (void *)a;
}


void nvm_slab_automove_extstore_free(void *arg) {
    slab_automove *a = (slab_automove *)arg;
    free(a->window_data);
    free(a->window_global);
    free(a);
...
void nvm_slab_automove_extstore_run(void *arg, int *src, int *dst) {
    slab_automove *a = (slab_automove *)arg;
    int n;
    struct window_data w_sum;
    *src = -1;
    *dst = -1;
    ...
    window_global_sum(a->window_global, &wg_sum, a->window_size);
    // fill after structs
    nvm_fill_slab_stats_automove(a->sam_after);
    a->window_cur++;
