...
typedef bool (*buf_swap_fn)(char* buf, size_t size, int pageVersion, int pageId, int offset, int local_nvm_slab_index, int local_nvm_per_thread_counter);
typedef bool (*item_swap)(char* buf, int pageVersion, int pageId, int offset,
...
typedef struct swap_buf{
   char* buf;
   struct swap_buf* next;
   unsigned int page_version;     /* page version for read mode */
   unsigned int len;     /* for both modes */
...
typedef struct swap_thread{
    pthread_mutex_t mutex;
    pthread_cond_t cond;
    unsigned int depth; // queue depth
    int8_t threadId; // queue depth
    int8_t stop; // queue depth

    swap_buf *queue;
    swap_buf *queue_tail;
    swap_buf *swapBufStack;
    void *engine;
}swap_thread;
...
extern __thread int storage_write_threadId;
extern int SSDread;
extern int asySSDread;
extern  uint64_t swapOp;
extern __thread uint64_t tmp_start_txn;
extern __thread int txn_threadId;
