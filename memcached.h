...
#define  MIN_VALUE_SIZE 256
//#define PRE_FIND
//#define DEBUG

#ifdef EXTSTORE
//#define use_alloc_pull_flags
#include "crc32c.h"
#endif
...
#define ITEM_ntotal(item) (sizeof(struct _stritem) + (item)->nkey + 1 \
         ...
         + (((item)->it_flags & ITEM_CFLAGS) ? sizeof(uint32_t) : 0) \
         + (((item)->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0))
#define ITEM_clsid(item) ((item)->slabs_clsid & ~(3<<6) & 255)
...
struct settings {
    ...
    unsigned int ext_free_memchunks[MAX_NUMBER_OF_SLAB_CLASSES * 2];
#endif

#ifdef ENABLE_NVM
    char* nvm_file;
    uint64_t nvm_limit;
    unsigned int nvm_block_size;
    unsigned int nvm_threadcount;
#endif
#ifdef TLS
...
#define IsHot(it)  ((current_time == it->time) && (it->read_count >= HOT_ITEM_R_COUNT))
#define IsWarm(it)  ((current_time == it->time) && (it->read_count >= HOT_ITEM_R_COUNT/2))
#define IsHotForSwap(it)  ((current_time==it->time) && (it->read_count >= HOT_ITEM_R_COUNT*3))
#define IsWarmForNoSwap(it)  ((current_time - it->time <= (1000000 / CLOCK_INTERVAL)) && (it->read_count >= HOT_ITEM_R_COUNT/2))
...
#ifdef ENABLE_NVM
    uint16_t itemFlag;
    void* io_ctx;            /* embedded extstore IO header */
#endif
...
void item_remove_setNeedSwap(item *item);
...
#ifdef RW_ITEM
void item_read_lock(uint32_t hv);
#endif
...
#ifdef M_LOCK
#define m_mutex_t Lock_t
#define m_mutex_lock(x) lock(x)
#define m_mutex_unlock(x) unlock(x)
#define m_mutex_try_lock(x) (try_lock(x)==1)
#define m_mutex_init(x) (*(x) = 0)

#elif defined(S_LOCK)
#define m_mutex_t pthread_spinlock_t
#define m_mutex_lock(x) pthread_spin_lock (x)
#define m_mutex_unlock(x)pthread_spin_unlock(x)
#define m_mutex_try_lock(x) (pthread_spin_trylock(x)==0)
#define m_mutex_init(x) pthread_spin_init(x, PTHREAD_PROCESS_PRIVATE)
#else
#define m_mutex_t pthread_mutex_t
#define m_mutex_lock(x) pthread_mutex_lock(x)
#define m_mutex_unlock(x) pthread_mutex_unlock(x)
#define m_mutex_try_lock(x) (pthread_mutex_trylock(x)==0)
#define m_mutex_init(x) pthread_mutex_init(x, NULL)
#endif
