...
void complete_nread_ascii(conn *c) {
    ...
    if ((it->it_flags & ITEM_CHUNKED) == 0) {
        if (strncmp(ITEM_data(it) + it->nbytes - 2, "\r\n", 2) == 0) {
            ...
            fprintf(stderr, " nbyte:%d, data:%p,  %d,%d\n", it->nbytes, (void *)ITEM_data(it),  (int)*(ITEM_data(it) + it->nbytes - 2), (int)*(ITEM_data(it) + it->nbytes - 1));
        }
    } else {
        char buf[2];
...
inline void process_get_command(conn *c, token_t *tokens, size_t ntokens, bool return_cas, bool should_touch) {
    char *key;
    size_t nkey;
    item *it;
            ...
            if (it) {
...

                {
                  ...
                  if (it->it_flags & ITEM_NVM) {
                      ...
                      item_hdr * hdr = (item_hdr*)ITEM_data(it);
                      if(hdr->cache_item != NULL){
                          resp->itemFlag &= (~ITEM_HDR);
                          char data[it->nbytes];
                          memcpy(data, ITEM_data(hdr->cache_item), it->nbytes);
                      } else if (storage_get_item(c, it, resp) != 0) {
                          item_remove(it);
                          goto stop;
                      }
#else
...
//                      fprintf(stderr, "it:%p, NVM_base:%p, NVM_len:%ld\n", (void*)it, (void*)NVM_base, NVM_len);
                ...
                if (settings.verbose > 1) {
                    int ii;
                    fprintf(stderr, ">%d io_pending:%p sending key", c->sfd, (void *)c->resp->io_pending);
...
#ifdef YCSB
                if((resp->itemFlag & ITEM_HDR) == 0){
                    // for hdr item, removed in storage_get_item
                    // must use resp->itemFlag as it->it_flags maybe changed
                    item_remove(it);
                }
                resp->item = NULL;
#else
...
//        fprintf(stderr, ">%d io_pending:%p end\n", c->sfd, (void *)c->resp->io_pending);
...
bool process_update_command_sep(void *storage, char* key, char* value, int key_n, int value_n, bool load){
    bool stored = false;
    uint32_t hv = hash(key, key_n);
    item* old_it = NULL;
    bool isHot = true, isWarm = true;
#ifdef PRE_FIND
    item_lock(hv);
    old_it = assoc_find(key, key_n, hv);
    isHot = (old_it != NULL) && IsHot(old_it);
//    isWarm =  (old_it != NULL) && IsWarm(old_it);
    item_unlock(hv);
#endif
#ifdef NVM_AIO
    isWarm = false;
    ...
    item *new_it = NULL;
    if(isHot){
        new_it = item_alloc(key, key_n, 0, 0, value_n + 2);
    }
    if(new_it!= NULL){
        memcpy(ITEM_data(new_it), value, value_n + 2);
        stored = true;
    } else{
#ifdef MEM_MOD
        item *cache_it;
#endif
        if(isWarm){
            new_it = do_item_alloc_nvm(key, key_n, 0, value_n + 2);

        }
        if(new_it!= NULL){
#ifdef MEM_MOD
            item_hdr *hdr = (item_hdr *) ITEM_data(new_it);
            item* nvm_item = (item*)hdr->nvmptr;
//            memcpy(ITEM_key(nvm_item), key, key_n);
#ifdef MEM_MOD
            cache_it = item_alloc(key, key_n, 0, 0, value_n + 2);
#endif
            if(cache_it != NULL){
...
//                cache_it->nkey = key_n;
#ifndef CACHE_NO_LINK
                item_link_q(cache_it);
#endif
            }else{
                hdr->cache_item = NULL;
                memcpy(ITEM_data(nvm_item), value, value_n + 2);
#ifdef M_STAT
...
#if defined(DRAM_CACHE) || defined(MEM_MOD)
            new_it = writeItToExt(storage, key, value, key_n, value_n + 2, hv);
            if(new_it != NULL){
                stored = true;
#ifdef M_STAT
            ...
            if((old_it->it_flags & ITEM_NVM)){
                ...
                if(hdr->cache_item != NULL) {
                    ...
                    if(hdr->cache_item->it_flags & ITEM_LINKED) {
                        if(item_try_unlink_q(hdr->cache_item)) {
...
#endif
#ifdef DRAM_CACHE
            if((old_it->it_flags & ITEM_HDR) || (old_it->it_flags & ITEM_NVM)){
                item_hdr *hdr = (item_hdr*) ITEM_data(old_it);
                if(hdr->cache_item != NULL) {
                    do_item_remove(hdr->cache_item);
                    hdr->cache_item = NULL;
                    item *cache_it = item_alloc(key, key_n, 0, 0, value_n + 2);
                    if (cache_it != NULL) {
                        memcpy(ITEM_data(cache_it), value, value_n + 2);
                        hdr->cache_item = cache_it;
                    }
...
void process_update_command(conn *c, token_t *tokens, const size_t ntokens, int comm, bool handle_cas) {
    ...
    if (it == 0) {
        ...
        if (comm == NREAD_SET) {
#ifndef YCSB  // no lock on item, remove is not safe
            ...
            if (it) {
                ...
                STORAGE_delete(c->thread->storage, it);
//                if(ITEM_clsid(it) == 2){
//                    fprintf(stderr, "(%ld), process_update_command replace :%p, key:%.30s\n",
//                            pthread_self(), (void *)it, ITEM_key(it));
//                }
                item_remove(it);
            }
#endif
    ...
    c->cmd = comm;
    conn_set_state(c, conn_nread);
//    fprintf(stderr, "process_update_command end\n");
}
