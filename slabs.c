...
#ifdef ENABLE_NVM
        mem_base = alloc_large_chunk(mem_limit + INDEX_SIZE);
#else
...
#else
    while (mem_malloced < (mem_limit + INDEX_SIZE)
            && (ptr = memory_allocate(len)) != NULL) {
#endif
        grow_slab_list(0);
...
#ifdef ENABLE_NVM
#ifndef NO_EXT
    if (( (p->size >= MIN_VALUE_SIZE ? ((p->slabs) >= ((mem_limit)/ ((uint64_t)settings.slab_page_size))) : false)
    || (p->slabs > 0 && g->slabs == 0)))  {
        if(false){
...
m_mutex_lock(&slabclass[0].slabs_meta[0].slab_lock);
    ...
    return 1;
        ...
        static int do_slabs_newslab_c(const unsigned int id, int slab_counter) {
            ...
            if (( (p->size >= MIN_VALUE_SIZE ? ((p->slabs) >= ((mem_limit + NVM_len)/ ((uint64_t)settings.slab_page_size))) : false)
            || (p->slabs > 0 && g->slabs == 0))) {
                if(false){
                    fprintf(stderr, "do_slabs_newslab id:%d, mem_malloced:%ld, mem_limit:%ld, p->slabs:%d, g->slabs:%d\n",
                            id, mem_malloced, mem_limit, p->slabs, g->slabs);
...
unsigned int slabs_available_chunks(const unsigned int id, bool *mem_flag,
    ...
    if(id < MAX_NUMBER_OF_SLAB_CLASSES){
        ...
        for (int i = slab_start; i < slab_end; ++i) {
            curr += p->slabs_meta[i].sl_curr;
            if (mem_flag != NULL){
                *mem_flag |= (p->slabs_meta[i].sl_curr) <= perFree;
            }
        }
        ret = curr;
#else
