...
#ifdef ENABLE_NVM
static slabclass_t nvm_slabclass[MAX_NUMBER_OF_SLAB_CLASSES];
static size_t nvm_limit = 0;
bool nvm_limit_reached = false;
__thread int slab_free_counter = 0;
__thread int slab_counter_index = 0;
__thread int slab_index = 0;
__thread int per_thread_counter = 0;
__thread int slab_span = 0;
__thread int slab_min = 0;

__thread enum THREAD_TYPE thread_type = 0;
...
#if  defined(EXTSTORE) || defined(ENABLE_NVM)
 ...
 */
#ifdef SLAB_PART
static pthread_mutex_t slabs_rebalance_lock[MAX_THREAD + 1];
#else
#ifdef M_LOCK
static m_mutex_t slabs_lock =0;
static pthread_mutex_t slabs_rebalance_lock = PTHREAD_MUTEX_INITIALIZER;
static m_mutex_t nvm_slabs_lock = 0;
#else
static pthread_mutex_t slabs_lock = PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t slabs_rebalance_lock = PTHREAD_MUTEX_INITIALIZER;
static pthread_mutex_t nvm_slabs_lock = PTHREAD_MUTEX_INITIALIZER;
#endif
#endif

extern int keyLen;
extern uint16_t free_slabs[MAX_THREAD];
...
static void do_slabs_free_c(void *ptr, const size_t size, unsigned int id, int slabIndex);
#ifdef FIX_KEY
static void do_slabs_free_for_split(void *ptr, const size_t size, unsigned int id, int slabIndex);
#endif
...
unsigned int slabs_fixup(char *chunk, const int border) {
#ifdef SLAB_PART
    fprintf(stderr, "SLAB_PART is defined, slabs_fixup should not be called error\n");
    exit(-1);
    return 0;
#else
    slabclass_t *p;
    ...
    if (it->it_flags == ITEM_SLABBED) {
        ...
        p->slots = it;
        if(p->slots == NULL){
            fprintf(stderr, "slab_rebalance_cut_free: slab:%d, p->slots = NULL\n", id);
        }
        p->sl_curr++;
...
void slabs_init(const size_t limit, const double factor, const bool prealloc, const uint32_t *slab_sizes, void *mem_base_external, bool reuse_mem) {
...
#ifdef tiny
    unsigned int size = sizeof(item) + sizeof(item_hdr);
    ...
    while (++i < MAX_NUMBER_OF_SLAB_CLASSES-1) {
        ...
        if (size > MIN_VALUE_SIZE) {
            if (size % NVM_ALIGN_BYTES)
                size += NVM_ALIGN_BYTES - (size % NVM_ALIGN_BYTES);
        }
        ...
        for (int j = 0; j < MAX_THREAD; ++j) {
            slabclass[i].slabs_meta[j].sl_curr = 0;
            slabclass[i].slabs_meta[j].slots = NULL;
            m_mutex_init(&slabclass[i].slabs_meta[j].slab_lock);
        }
#endif
    }
#ifdef SLAB_PART
    for (int j = 0; j < MAX_THREAD; ++j) {
        m_mutex_init(&slabclass[0].slabs_meta[j].slab_lock);
    }
#endif
...
#ifdef SLAB_PART
    for (int j = 0; j < MAX_THREAD; ++j) {
        pthread_mutex_init(&slabs_rebalance_lock[j], NULL);
    }
    pthread_mutex_init(&slabs_rebalance_lock[MAX_THREAD], NULL);
#endif
}
...
void slabs_prefill_global_with_nvm(char* src, uint64_t size) {
    slabclass_t *p = &slabclass[0];
    uint64_t len = settings.slab_page_size;
    uint64_t index = 0;
    while (index + len < size) {
        ...
        // It's probably good enough to cast it and just zero slabs_clsid, but
        // this is extra paranoid.
        p->slab_list[p->slabs++] = src + index;
        index += len;
//        fprintf(stderr, "slabs_prefill_global_with_nvm p->slab_list:%d, p:%p\n", p->slabs - 1, (void *)(src + index));
    }
//    mem_limit += size;
//    mem_malloced += size;
...
#ifdef FIX_KEY
        do_slabs_free_for_split(ptr, 0, id, slabIndex);
#else
...
static void split_slab_page_into_freelist_c(char *ptr, const unsigned int id, int slab_counter) {
    slabclass_t *p;
    if(id & NVM_SLAB){
        p = &nvm_slabclass[id & (~NVM_SLAB)];
    } else{
    ...
    int x;
    int slabIndex = slab_counter % MAX_THREAD;
    for (x = 0; x < p->perslab; x++) {
#ifdef FIX_KEY
        do_slabs_free_for_split(ptr, 0, id, slabIndex);
#else
...
static void *get_page_from_global_pool(void) {
    ...
    p->slabs--;
//    fprintf(stderr, "get_page_from_global_pool p->slabs:%d, p:%p\n", p->slabs, (void *)ret);
    return ret;
}

#ifdef ENABLE_NVM
static int nvm_grow_slab_list (const unsigned int id) {
    slabclass_t *p = &nvm_slabclass[id];
    if (p->slabs == p->list_size) {
        size_t new_size =  (p->list_size != 0) ? p->list_size * 2 : 16;
        void *new_list = realloc(p->slab_list, new_size * sizeof(void *));
        if (new_list == 0) return 0;
...
/* Fast FIFO queue */
static void *nvm_get_page_from_global_pool(void) {
    slabclass_t *p = &nvm_slabclass[SLAB_GLOBAL_PAGE_POOL];
    if (p->slabs < 1) {
        return NULL;
    }
    char *ret = p->slab_list[p->slabs - 1];
...
//#define DO_DEBUG
static int nvm_do_slabs_newslab_c(const unsigned int id, int slab_counter) {
    slabclass_t *p = &nvm_slabclass[id];
    slabclass_t *g = &nvm_slabclass[SLAB_GLOBAL_PAGE_POOL];
//    int len = settings.nvm_block_size;
    char *ptr;

    if (g->slabs == 0 || (slabclass[SLAB_GLOBAL_PAGE_POOL].slabs <= (nvm_slabclass[id].perslab / slabclass[1].perslab))) {
#ifdef DO_DEBUG
        fprintf(stderr, "nvm_do_slabs_newslab p->slabs:%d, g->slabs:%d\n",
                 p->slabs, g->slabs);
        MEMCACHED_SLABS_SLABCLASS_ALLOCATE_FAILED(id);
#endif
        nvm_limit_reached = true;
        return 0;
    }
#ifdef SLAB_PART
    m_mutex_lock(&nvm_slabclass[0].slabs_meta[0].slab_lock);
#endif
    if ((nvm_grow_slab_list(id) == 0) ||
    (((ptr = nvm_get_page_from_global_pool()) == NULL))) {
#ifdef DO_DEBUG
        fprintf(stderr, "nvm_do_slabs_newslab p->slabs:%d, g->slabs:%d\n",
                p->slabs, g->slabs);
#endif
//        nvm_limit_reached = true;
        MEMCACHED_SLABS_SLABCLASS_ALLOCATE_FAILED(id | NVM_SLAB);
#ifdef SLAB_PART
        m_mutex_unlock(&nvm_slabclass[0].slabs_meta[0].slab_lock);
#endif
        return 0;
    }
    p->slab_list[p->slabs++] = ptr;
...
void nvm_slabs_init(void* data, const size_t limit, const bool prealloc, bool reuse_mem) {
    int i = POWER_SMALLEST - 1;
    nvm_limit = limit;

    memset(nvm_slabclass, 0, sizeof(nvm_slabclass));

    while (++i < MAX_NUMBER_OF_SLAB_CLASSES-1) {
        if(slabclass[i].size == 0){
            break;
        }
        nvm_slabclass[i].size = slabclass[i].size ;
        if (nvm_slabclass[i].size > MIN_VALUE_SIZE) {
            if (nvm_slabclass[i].size % NVM_ALIGN_BYTES)
                nvm_slabclass[i].size += NVM_ALIGN_BYTES - (nvm_slabclass[i].size % NVM_ALIGN_BYTES);
        }
        nvm_slabclass[i].perslab = settings.nvm_block_size / nvm_slabclass[i].size;
        if (settings.verbose > 1) {
            fprintf(stderr, "nvm slab class %3d: chunk size %9u perslab %7u\n",
                    i, slabclass[i].size, slabclass[i].perslab);
        }
    }
    nvm_slabclass[0].size = settings.nvm_block_size;
    nvm_slabclass[0].perslab = 1;

    slabclass_t *p = &nvm_slabclass[SLAB_GLOBAL_PAGE_POOL];
    p->list_size = limit / settings.nvm_block_size;
    p->slab_list = malloc(sizeof (void *) * p->list_size);
    p->slabs = 0;
    for (size_t j = 0; j < limit; j += settings.nvm_block_size) {
        p->slab_list[p->slabs ++ ] = ((char*)data) + j;
#ifdef SLAB_PART
        for (int j = 0; j < MAX_THREAD; ++j) {
            p->slabs_meta[j].sl_curr = 0;
            p->slabs_meta[j].slots = NULL;
            m_mutex_init(&p->slabs_meta[j].slab_lock);
        }
#endif
    }
    if (prealloc && !reuse_mem) {
        int i;
        unsigned int prealloc = 0;
        ...
        for (i = POWER_SMALLEST; i < MAX_NUMBER_OF_SLAB_CLASSES; i++) {
            if (++prealloc > power_largest)
                break;
            if (nvm_do_slabs_newslab_c(i, 0) == 0) {
                fprintf(stderr, "Error while preallocating slab memory!\n"
                                "If using -L or other prealloc options, max memory must be "
                                "at least %d megabytes.\n", power_largest);
                exit(1);
...
void prealloc_nvm_slab(int id, int slab_counter){
#ifndef SLAB_PART
    m_mutex_lock(&nvm_slabs_lock);
#endif
    while (nvm_do_slabs_newslab_c(id, slab_counter) != 0){

    }
#ifndef SLAB_PART
    m_mutex_unlock(&nvm_slabs_lock);
#endif
}

void nvm_fill_slab_stats_automove(slab_stats_automove *am) {
    int n;
    #ifdef SLAB_PART
    m_mutex_t *lock_p = &nvm_slabclass[0].slabs_meta[0].slab_lock;
#else
    m_mutex_t *lock_p = &nvm_slabs_lock;
#endif
    m_mutex_lock(lock_p);
    for (n = 0; n < MAX_NUMBER_OF_SLAB_CLASSES; n++) {
        slabclass_t *p = &nvm_slabclass[n];
        ...
        for (int j = 0; j < MAX_THREAD; ++j) {
            cur->free_chunks += p->slabs_meta[j].sl_curr;
        }
#else
...
#ifdef SLAB_PART
//fprintf(stderr, "do_slabs_newslab %ld, apply slab_lock:%d\n", pthread_self(), MAX_THREAD);
m_mutex_lock(&slabclass[0].slabs_meta[0].slab_lock);
#endif
    if ((grow_slab_list(id) == 0) ||
        (((ptr = get_page_from_global_pool()) == NULL) &&
        ((ptr = memory_allocate((size_t)len)) == 0))) {
#ifdef DO_DEBUG
        fprintf(stderr, "do_slabs_newslab mem_malloced:%ld, mem_limit:%ld, p->slabs:%d, g->slabs:%d\n",
                mem_malloced, mem_limit, p->slabs, g->slabs);
#endif
        mem_limit_reached = true;
        MEMCACHED_SLABS_SLABCLASS_ALLOCATE_FAILED(id);
#ifdef SLAB_PART
        m_mutex_unlock(&slabclass[0].slabs_meta[0].slab_lock);
#endif
        return 0;
    }
    p->slab_list[p->slabs++] = ptr;
...
#ifdef SLAB_PART
        extern char* NVM_base;
        extern size_t NVM_len;
extern size_t NVM_size;
...
//            slabclass_t *g = &slabclass[SLAB_GLOBAL_PAGE_POOL];
            ...
            if(p->size < MIN_VALUE_SIZE){
                ...
                // blocking startup all at once.
                memset(ptr, 0, (size_t)len);
                split_slab_page_into_freelist_c(ptr, id, slab_counter);
                return 1;
            }
#endif
...
static void *do_slabs_alloc(const size_t size, unsigned int id,
    ...
    int slabIndex = 0;
//    fprintf(stderr, "(%ld), slabs_alloc: slab_index:%d\n", pthread_self(), slab_index);
    ...
    assert(p->slabs_meta[slabIndex].sl_curr == 0 || (((item *)p->slabs_meta[slabIndex].slots)->it_flags & ITEM_SLABBED));

    assert(size <= p->size);
    /* fail unless we have space at the end of a recently allocated page,
       we have something on our freelist, or we could allocate a new page */
    if (p->slabs_meta[slabIndex].sl_curr == 0 && flags != SLABS_ALLOC_NO_NEWPAGE) {
        if(id & NVM_SLAB ){
#ifdef ENABLE_NVM
            if(!nvm_limit_reached){
                nvm_do_slabs_newslab_c(id & (~NVM_SLAB), 0);
            }
#endif
        } else{
            do_slabs_newslab(id);
    ...
    if (p->slabs_meta[slabIndex].sl_curr != 0) {
        /* return off our freelist */
        it = (item *)p->slabs_meta[slabIndex].slots;
        p->slabs_meta[slabIndex].slots = it->next;
        ...
        if(p->slabs_meta[slabIndex].sl_curr > 0 && p->slabs_meta[slabIndex].slots == NULL){
            fprintf(stderr, "(%ld), do_slabs_alloc: slab:%d, slab_index:%d, sl_curr:%d, slot:%p, it:%p error\n",
                    ...
                    (void *)p->slabs_meta[slabIndex].slots, (void *)it);
            BACKTRACE_T
            exit(0);
        }
    ...
    if(id & NVM_SLAB){
        p = &nvm_slabclass[id & (~NVM_SLAB)];
//        fprintf(stderr, "do_slabs_alloc: slab:%d, p->sl_curr:%d\n", id, p->sl_curr);
    } else{
        p = &slabclass[id];
    }
    ...
    if (p->sl_curr == 0 && flags != SLABS_ALLOC_NO_NEWPAGE) {
        if(id & NVM_SLAB){
#ifdef ENABLE_NVM
            nvm_do_slabs_newslab(id & (~NVM_SLAB));
#endif
        } else{
            do_slabs_newslab(id);
        }
...
#endif
static void *do_slabs_alloc_c(const size_t size, unsigned int id,
                            unsigned int flags, int slab_counter) {
    slabclass_t *p;
    void *ret = NULL;
    item *it = NULL;
#ifdef SLAB_PART
...
#ifdef ENABLE_NVM
            if(!nvm_limit_reached){
                nvm_do_slabs_newslab_c(id & (~NVM_SLAB), slab_counter);
            }
#endif
        } else{
            do_slabs_newslab_c(id, slab_counter);
        }
    }
    ...
    if (p->slabs_meta[slabIndex].sl_curr != 0) {
        ...
        if((it->it_flags & ITEM_SLABBED) == 0){
            fprintf(stderr, "(%ld), do_slabs_alloc_c: slab:%d,  ptr:%p, slabIndex:%d, it->flag:%d ref:%d error\n",
        ...
        it->refcount = 1;
        it->read_count = 0;
        //reset slabs_clsid to clear the old slab_index
        it->slabs_clsid = id & (~NVM_SLAB);
        p->slabs_meta[slabIndex].sl_curr--;
        if((it->it_flags & (~ITEM_NVM)) !=0 ){
            fprintf(stderr, "(%ld), do_slabs_alloc_c: slab:%d, slabIndex:%d, sl_curr:%d, slot:%p, it:%p, it->flag:%d error\n",
                    pthread_self(), id, slabIndex,  p->slabs_meta[slabIndex].sl_curr,
        ...
        assert(!(p->slabs_meta[slabIndex].sl_curr > 0 && p->slabs_meta[slabIndex].slots == NULL));
        if(p->slabs_meta[slabIndex].sl_curr > 0 && p->slabs_meta[slabIndex].slots == NULL){
            fprintf(stderr, "(%ld), do_slabs_alloc_c: slab:%d, slab_index:%d, sl_curr:%d, slot:%p, it:%p error\n",
                    pthread_self(), id, slabIndex,  p->slabs_meta[slabIndex].sl_curr,
                    (void *)p->slabs_meta[slabIndex].slots, (void *)it);
            BACKTRACE_T
            exit(0);
...
static void do_slabs_free_chunked(item *it, const size_t size) {
    ...
    if (it->next) it->next->prev = it;
    p->slots = it;
//    if( p->sl_curr > 0 &&((item *)p->slots)->next == NULL){
//        fprintf(stderr, "do_slabs_free_chunked: slab:%d, p->slots->next = NULL, p->sl_curr:%d\n", chunk->orig_clsid, p->sl_curr);
//    }
...
/* slabIndex need to be passed in, as slab_counter will be changed by do_item_alloc*/
static void do_slabs_free_for_split(void *ptr, const size_t size, unsigned int id, int slabIndex) {
    slabclass_t *p;
    item *it = NULL;
#ifdef SLAB_PART
    if(id & NVM_SLAB){
        ...
        if (((id&(~NVM_SLAB)) < POWER_SMALLEST) ||((id&(~NVM_SLAB)) > power_largest) || nvm_limit_reached)
            return;
        MEMCACHED_SLABS_FREE(size, id, ptr);
        p = &nvm_slabclass[id & (~NVM_SLAB)];
        int count = 0;
        while (it == NULL){
            it = do_item_alloc(NULL, keyLen, 0, 0, sizeof(item_hdr));
            count ++;
            if(count % MAX_THREAD == 0){
#ifdef DRAM_TEST
                nvm_limit_reached = true;
#endif
                fprintf(stderr, "(%ld), do_slabs_free_for_split:nvm_limit_reached \n", pthread_self());
                return;
            }
        }

//        fprintf(stderr, "(%ld), do_slabs_free_for_split: slab:%d,  ptr:%p, slab_index:%d,nkey:%d, keyLen:%d\n",
//                pthread_self(),  id, (void *)it, slabIndex, it->nkey, keyLen);
        if(it->it_flags & ITEM_SLABBED){
            fprintf(stderr, "(%ld), do_slabs_free_for_split: slab:%d, curr:%d,  "
                            "ptr:%p, slabIndex:%d, it->next:%p, it->flag:%d,  error\n",
                            pthread_self(),  id, p->slabs_meta[slabIndex].sl_curr,
                            ptr, slabIndex, (void *)it->next, it->it_flags);
            BACKTRACE_T
            exit(0);
        }
        ...
        it->it_flags = ITEM_SLABBED | ITEM_NVM;
        it->slabs_clsid = id & (~NVM_SLAB);
        it->prev = 0;
        it->next = p->slabs_meta[slabIndex].slots;
        if (it->next) it->next->prev = it;
        p->slabs_meta[slabIndex].slots = it;
        p->slabs_meta[slabIndex].sl_curr++;

        if(p->slabs_meta[slabIndex].sl_curr > 1 && it->next == NULL){
            fprintf(stderr, "(%ld), do_slabs_free_for_split: slab:%d, curr:%d,  ptr:%p, slab_index:%d, it->next:%p error\n",
...
//        int slabIndex = slab_counter % MAX_THREAD;
        ...
        if(it->it_flags & ITEM_SLABBED){
            fprintf(stderr, "(%ld), do_slabs_free_for_split: slab:%d, curr:%d,  "
                            "ptr:%p, slabIndex:%d, it->next:%p, it->flag:%d\n",
                            pthread_self(),  id, p->slabs_meta[slabIndex].sl_curr,
                            ptr, slabIndex, (void *)it->next, it->it_flags);
            BACKTRACE_T
            exit(0);
...
static void do_slabs_free_c(void *ptr, const size_t size, unsigned int id, int slabIndex) {
    ...
    item *it = NULL;
#ifdef SLAB_PART
    assert((id&(~NVM_SLAB)) >= POWER_SMALLEST && ((id&(~NVM_SLAB)) <= power_largest));
    MEMCACHED_SLABS_FREE(size, id, ptr);
    ...
    if(((it->it_flags & ITEM_SLABBED) || (it->it_flags & ITEM_LINKED))){
        fprintf(stderr, "(%ld), do_slabs_free_c: slab:%d, curr:%d,  "
                        "ptr:%p, slabIndex:%d, it->next:%p, it->flag:%d, thread_type:%d, ref:%d , hv:%d, error\n",
                        pthread_self(),  id, p->slabs_meta[slabIndex].sl_curr,
                        ptr, slabIndex, (void *)it->next, it->it_flags, thread_type, it->refcount, hash(ITEM_key(it), it->nkey));
        BACKTRACE_T
        exit(0);
    }

    it->it_flags = id & NVM_SLAB ? ITEM_SLABBED | ITEM_NVM :  ITEM_SLABBED;
    it->slabs_clsid = ((id & (~NVM_SLAB))) | (thread_type * POWER_LARGEST) ;
    it->prev = 0;
    it->next = p->slabs_meta[slabIndex].slots;
    if (it->next) it->next->prev = it;
    p->slabs_meta[slabIndex].slots = it;
    ...
    if(id & NVM_SLAB){
        p = &nvm_slabclass[id & (~NVM_SLAB)];
//        fprintf(stderr, "do_slabs_free, id:%d, p->sl_curr:%d\n", id & (~NVM_SLAB), p->sl_curr);
    } else{
        p = &slabclass[id];
    }
...
void fill_slab_stats_automove(slab_stats_automove *am) {
    int n;
#ifdef SLAB_PART
    m_mutex_t *lock_p =&slabclass[0].slabs_meta[0].slab_lock;
#else
    m_mutex_t *lock_p = &slabs_lock;
#endif
    m_mutex_lock(lock_p);
    for (n = 0; n < MAX_NUMBER_OF_SLAB_CLASSES; n++) {
    ...
    for(i = POWER_SMALLEST; i <= power_largest; i++) {
        ...
        if (p->slabs != 0) {
            ...
            APPEND_NUM_STAT(i, "used_chunks", "%u",
                            slabs*perslab - free);
            APPEND_NUM_STAT(i, "free_chunks", "%u", free);
#else
            APPEND_NUM_STAT(i, "used_chunks", "%u",
...
void *slabs_alloc(size_t size, unsigned int id,
        unsigned int flags) {
    void *ret = NULL;
    m_mutex_t* lock_p = NULL;
    lock_p = (id & NVM_SLAB) ? (&nvm_slabs_lock)
            : (&slabs_lock);
    m_mutex_lock(lock_p);

    ret = do_slabs_alloc_c(size, id, flags, 0);
    m_mutex_unlock(lock_p);
    return ret;
}
#endif
...
void *slabs_alloc_c(size_t size, unsigned int id,
                    unsigned int flags, int slab_counter, int local_slabNum) {
    void *ret = NULL;
    m_mutex_t* lock_p = NULL;
#ifdef SLAB_PART
    if(id & NVM_SLAB){
        if(nvm_slabclass[id & (~NVM_SLAB)].slabs_meta[slab_counter].sl_curr == 0){
            return NULL;
        }
        lock_p = &nvm_slabclass[(id & (~NVM_SLAB))].slabs_meta[slab_counter].slab_lock;
        m_mutex_lock(lock_p);
    } else{
        lock_p = &slabclass[(id)].slabs_meta[slab_counter].slab_lock;
        m_mutex_lock(lock_p);
    }
#else
    lock_p = (id & NVM_SLAB) ? (&nvm_slabs_lock)
...
void slabs_free(void *ptr, size_t size, unsigned int id) {
    m_mutex_t* lock_p = NULL;
    int slabIndex = slab_free_counter + slab_index;
#ifdef SLAB_PART
    if(id & NVM_SLAB){
        lock_p= (&nvm_slabclass[(id & (~NVM_SLAB))].slabs_meta[slabIndex].slab_lock);
    } else{
        lock_p= (&slabclass[(id)].slabs_meta[slabIndex].slab_lock);
    }
#else
    lock_p = (id & NVM_SLAB) ? (&nvm_slabs_lock)
            : (&slabs_lock);
    ...
    do_slabs_free_c(ptr, size, id, slabIndex);

    m_mutex_unlock(lock_p);
#ifdef SLAB_PART
    slab_free_counter =  (slab_free_counter+ 1)%per_thread_counter;
#endif
}
...
bool slabs_adjust_mem_limit(size_t new_mem_limit) {
    bool ret;
#ifdef SLAB_PART
    fprintf(stderr, "SLAB_PART is defined, slabs_adjust_mem_limit should not be called error\n");
    exit(-1);
#else
    m_mutex_lock(&slabs_lock);
    ret = do_slabs_adjust_mem_limit(new_mem_limit);
    m_mutex_unlock(&slabs_lock);
#endif
    return ret;
}
...
bool if_nvm_limit_reached(void){
    slabclass_t * g = &nvm_slabclass[SLAB_GLOBAL_PAGE_POOL];
#ifdef SLAB_PART
    bool reached = (g->slabs) <= (nvm_limit * settings.slab_automove_freeratio / (g->size * g->perslab));
    bool slabReached = false;

    if(reached){
        for (int x = POWER_SMALLEST; x < MAX_NUMBER_OF_SLAB_CLASSES && (!slabReached); x++) {
            slabclass_t * p = &nvm_slabclass[x];
            if(p->slabs == 0){
                continue;
            }
            int curr = 0;
            ...
            slabReached |= ((p->slabs * p->perslab * settings.slab_automove_freeratio) >= curr);
        }
    }
    ...
    if(reached){
        for (int x = POWER_SMALLEST; x < MAX_NUMBER_OF_SLAB_CLASSES && (!slabReached); x++) {
            slabclass_t * p = &nvm_slabclass[x];
            if(p->slabs > 0){
                slabReached |= ((p->slabs * settings.slab_automove_freeratio * p->perslab) >= p->sl_curr);
//                if(slabReached){
//                    fprintf(stderr, "if_nvm_limit_reached: slab:%d, p->slabs:%d,  p->perslab:%d, p->sl_curr:%d\n",
//                            x, p->slabs, p->perslab, p->sl_curr);
//                }
            }
        }
    }
    if(reached && (++count) % 1000 == 0){
        fprintf(stderr, "if_nvm_limit_reached: g->slabs:%d, slabReached:%d, p->slabs:%d, sl_curr:%d\n",
                g->slabs, slabReached, nvm_slabclass[12].slabs, nvm_slabclass[12].sl_curr);
    }
#endif
...
#ifdef ENABLE_NVM
            /*
            *mem_flag = (mem_malloced >=( mem_limit * (1 - settings.slab_automove_freeratio)) );
            double freeRatio;
            if(*mem_flag){
                slabclass_t * g = &slabclass[SLAB_GLOBAL_PAGE_POOL];
                freeRatio = (settings.slab_automove_freeratio  +(1- DATA_DRAM_RATIO)) > 0.8? 0.8
                        : (settings.slab_automove_freeratio  + (1- DATA_DRAM_RATIO));
                *mem_flag = ((g->slabs)
                        <= (mem_limit* freeRatio/ settings.slab_page_size));
            }
...
#ifdef SLAB_PART
//        int perFree = (slabs * p->perslab) * settings.slab_automove_freeratio / slabNum;
        for (int i = slab_start; i < slab_end; ++i) {
            curr += p->slabs_meta[i].sl_curr;
            if (mem_flag != NULL){
                *mem_flag |= (p->slabs_meta[i].sl_curr) <=  1;
...
//        unsigned int hold_free = (slabs * p->perslab) * settings.slab_automove_freeratio;
        ...
        if (mem_flag != NULL){
            ...
            (*mem_flag) = (g->slabs) <= (nvm_limit * settings.slab_automove_freeratio / g->size / g->perslab);


            (*mem_flag) &= ((p->slabs * settings.slab_automove_freeratio) >= curr / p->perslab);
             */
#ifndef SLAB_PART
            *mem_flag = (curr < hold_free);
#endif
...
#ifdef SLAB_PART
    fprintf(stderr, "SLAB_PART is defined, slabs_mlock should not be called error\n");
    exit(-1);
#else
    m_mutex_lock(&slabs_lock);
#endif
...
#ifdef SLAB_PART
    fprintf(stderr, "SLAB_PART is defined, slabs_munlock should not be called error\n");
    exit(-1);
#else
    m_mutex_unlock(&slabs_lock);
#endif
...
#ifdef SLAB_PART
    fprintf(stderr, "SLAB_PART is defined, slab_rebalance_start should not be called\n");
    exit(-1);
#else
    slabclass_t *s_cls;
    int no_go = 0;
...
#ifdef SLAB_PART
    fprintf(stderr, "SLAB_PART is defined, slab_rebalance_cut_free should not be called\n");
    exit(-1);
#else
    if (s_cls->slots == it) {
        s_cls->slots = it->next;
...
#ifdef SLAB_PART
    fprintf(stderr, "SLAB_PART is defined, slab_rebalance_move should not be called\n");
    exit(-1);
#else
    slabclass_t *s_cls;
    int was_busy = 0;
        ...
        switch (status) {
            case MOVE_FROM_LRU:
                ...
                || item_is_flushed(it)) {
                    ...
                    save_item = 1;
                    new_it = slabs_alloc(ntotal, slab_rebal.s_clsid, 0);
                }
#else
                if ((it->exptime != 0 && it->exptime < current_time)
                    || item_is_flushed(it)) {
...
#ifdef SLAB_PART
    fprintf(stderr, "SLAB_PART is defined, slab_rebalance_finish should not be called\n");
    exit(-1);
#else
    slabclass_t *s_cls;
    slabclass_t *d_cls;
...
#ifdef SLAB_PART
    fprintf(stderr, "SLAB_PART is defined, slab_rebalance_thread should not be called error\n");
    exit(-1);
#else
    int was_busy = 0;
    int backoff_timer = 1;
...
#ifdef SLAB_PART
    fprintf(stderr, "SLAB_PART is defined, do_slabs_reassign should not be called\n");
    exit(-1);
    return 0;
#else
    bool nospare = false;
...
#ifdef SLAB_PART
    fprintf(stderr, "SLAB_PART is defined, slabs_reassign should not be called error\n");
    exit(-1);
#else
...
#ifdef SLAB_PART
    fprintf(stderr, "SLAB_PART is defined, slabs_rebalancer_pause should not be called error\n");
    exit(-1);
#else
...
#ifdef SLAB_PART
    fprintf(stderr, "SLAB_PART is defined, slabs_rebalancer_resume should not be called error\n");
    exit(-1);
#else
    pthread_mutex_unlock(&slabs_rebalance_lock);
#endif
...
#ifdef SLAB_PART
    fprintf(stderr, "SLAB_PART is defined, stop_slab_maintenance_thread should not be called\n");
    exit(-1);
#else
    mutex_lock(&slabs_rebalance_lock);
...
void print_slab(int  slab_id){
#ifdef SLAB_PART
    if(slab_id & NVM_SLAB){
        for (int  i = 0; i < slabNum*2; ++i) {
            print_nvm_slab_tail((slab_id & (~NVM_SLAB))| (i * POWER_LARGEST));
        }
        for (int i = 0; i < slabNum*2; ++i) {
            fprintf(stderr, "print_nvm_slab: id:%d, slab:%d,  sl_curr[%d]:%d\n",
                    slab_id, nvm_slabclass[slab_id& (~NVM_SLAB)].slabs, i, nvm_slabclass[slab_id& (~NVM_SLAB)].slabs_meta[i].sl_curr);
        }

    } else{
        for (int i = 0; i < slabNum * 2; ++i) {
            print_slab_tail(slab_id | (i * POWER_LARGEST));
        }
        for (int i = 0; i < slabNum * 2; ++i) {
            fprintf(stderr, "print_slab id:%d: slab:%d,  perslab:%d, sl_curr[%d]:%d,alloc_free_flag:%d\n",
                    slab_id, slabclass[slab_id].slabs, slabclass[slab_id].perslab, i,
                    slabclass[slab_id].slabs_meta[i].sl_curr,0);
        }
    }
#else
    if(slab_id & NVM_SLAB){
...
#else
    print_nvm_slab_tail(slab_id&(~NVM_SLAB));
#endif
        slabclass_t * p = &nvm_slabclass[slab_id &(~NVM_SLAB)];
        fprintf(stderr, "print_nvm_slab: slab:%d,  p->slabs:%d, p->sl_curr:%d\n",
                slab_id, p->slabs, p->sl_curr);
    } else{
#ifdef LRU_COUNTER
...
#else
        print_slab_tail(slab_id);
#endif
        slabclass_t * p = &slabclass[slab_id];
        fprintf(stderr, "print_slab: slab:%d,  p->slabs:%d, p->sl_curr:%d, p->perslab:%d\n",
                slab_id, p->slabs, p->sl_curr, p->perslab);
//        p = &slabclass[12];
//        fprintf(stderr, "print_slab: slab:12,  p->slabs:%d, p->sl_curr:%d\n",
//                p->slabs, p->sl_curr);
//        p = &slabclass[2];
//        fprintf(stderr, "print_slab: slab:2,  p->slabs:%d, p->sl_curr:%d\n",
//                p->slabs, p->sl_curr);
    }
#endif
